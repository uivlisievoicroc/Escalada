...
class RateLimiter:
    ...
    def __init__(self, 
        ...
        self.request_history: Dict[int, Dict] = defaultdict(lambda: {
            'requests': [],
            'blocked_until': 0
        })
    ...
    def check_rate_limit(self, box_id: int, command_type: str) -> Tuple[bool, str]:
        ...
        if len(recent_requests) >= self.max_per_second:
            ...
            logger.warning(f'Box {box_id} exceeded per-second limit ({self.max_per_second} req/sec)')
            return False, f'Rate limit exceeded (too many requests per second)'
        
        # Check per-minute limit
        if len(requests) >= self.max_per_minute:
            history['blocked_until'] = current_time + self.block_duration
            logger.warning(f'Box {box_id} exceeded per-minute limit ({self.max_per_minute} req/min)')
