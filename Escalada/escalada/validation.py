...
class ValidatedCmd(BaseModel):
    ...
    delta: Optional[float] = Field(None, ge=-10.0, le=10.0, description="Progress delta (-10 to +10)")
    score: Optional[float] = Field(None, ge=0.0, le=100.0, description="Score (0-100)")
    competitor: Optional[str] = Field(None, min_length=1, max_length=255, description="Competitor name")
    registeredTime: Optional[float] = Field(None, ge=0, le=3600, description="Registered time (0-3600 seconds)")
    ...
    timerPreset: Optional[str] = Field(None, max_length=20, description="Timer preset (e.g., '05:00')")
    ...
    def validate_type(cls, v: str) -> str:
        ...
        allowed_types = {
            'START_TIMER', 'STOP_TIMER', 'RESUME_TIMER',
    ...
    def validate_cmd_fields(self) -> Self:
        """Validate command has required fields based on type"""
        cmd_type = self.type
        
        # Type-specific field validation
        if cmd_type == 'PROGRESS_UPDATE':
            if self.delta is None:
                raise ValueError('PROGRESS_UPDATE requires delta field')
        
        elif cmd_type == 'SUBMIT_SCORE':
            if self.score is None:
                raise ValueError('SUBMIT_SCORE requires score field')
            if self.competitor is None:
                raise ValueError('SUBMIT_SCORE requires competitor field')
        
        elif cmd_type == 'INIT_ROUTE':
            ...
            if self.competitors is None:
                raise ValueError('INIT_ROUTE requires competitors')
        
        elif cmd_type == 'TIMER_SYNC':
            if self.remaining is None:
                raise ValueError('TIMER_SYNC requires remaining field')
...
class RateLimitConfig:
    ...
    MAX_REQUESTS_PER_MINUTE = 60  # Per box per minute
    MAX_REQUESTS_PER_SECOND = 10   # Per box per second
    BLOCKED_DURATION_SECONDS = 60  # How long to block after limit exceeded
    
    # Per-command-type limits (per minute)
    COMMAND_LIMITS = {
        'START_TIMER': 30,
        'STOP_TIMER': 30,
        'RESUME_TIMER': 30,
